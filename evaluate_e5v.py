# source activate /share/huaying/envs/memovideo
import sys
...
def build_dataset(file: str) -> Dataset:
    ...
    def process_query_example(query):
        ...
        assert len(gt_clips) > 0, f"gt_clips is empty {qtext}"
...
if __name__ == "__main__":
    ...
    processor = LlavaNextProcessor.from_pretrained('/share/huaying/pretrained_model/e5-v')
    model = LlavaNextForConditionalGeneration.from_pretrained('/share/huaying/pretrained_model/e5-v', torch_dtype=torch.float16).cuda()

    img_prompt = llama3_template.format('<image>\nSummary above image in one word: ')
    text_prompt = llama3_template.format('<sent>\nSummary above sentence in one word: ')
    text_img_prompt = "<image>Modify this image with \"<sent>\", desribe modified image in one word: "


    output_dir = './languagebind/benchmark/test_grounding/result/e5v/'
    os.makedirs(output_dir,exist_ok=True)
    ...
    for file in os.listdir(benchmark_dir):
        try:
            ...
            eval_dataset.query_examples = eval_dataset.query_examples
            
            # ok=0
            # if os.path.exists(f'{output_dir}/{file.split(".")[0]}_index.pkl'):
            #     index_embeddings = pickle.load(open(f'{output_dir}/{file.split(".")[0]}_index.pkl','rb'))
            #     if index_embeddings.shape[0]==len(eval_dataset.index_examples):
            #         ok=1
            ok = 0
            ...
            if not ok:
                ...
                for i in tqdm(range(0, len(eval_dataset.index_examples), args.batch_size)):
                    ...
                    images = [Image.open(img) for img in batch_video]
                    video_inputs = processor([img_prompt]*len(images), images, return_tensors="pt", padding=True)
                    video_inputs = {k: v.to('cuda') for k, v in video_inputs.items()}

                    with torch.no_grad():
                        video_embed = model(**video_inputs, output_hidden_states=True, return_dict=True).hidden_states[-1][:, -1, :]
                        video_embed = F.normalize(video_embed, dim=-1)

                    index_embeddings.append(video_embed.detach().cpu().numpy())
            ...
            for i in tqdm(range(0, len(eval_dataset.query_examples))):
                ...
                if batch[0].qimage is not None:
                    query_text = [text_img_prompt.replace('<sent>', q.qtokens) for q in batch]
                    query_image = [Image.open(q.qimage) for q in batch]
                    # print(query_text)
                    # print(query_image)
                    inputs = processor(query_image, query_text, return_tensors="pt", padding=True)
                    inputs = {k: v.to('cuda') for k, v in inputs.items()}
                    with torch.no_grad():
                        query_embeds = model(**inputs, output_hidden_states=True, return_dict=True).hidden_states[-1][:, -1, :]
                        query_embeds = F.normalize(query_embeds, dim=-1)
                        query_embeds = query_embeds.detach().cpu().numpy()

                else:
                    query_text = [text_img_prompt.replace('<sent>', q.qtokens) for q in batch]
                    text_inputs = processor(query_text, None, return_tensors="pt", padding=True)
                    ...
                    query_embeds = text_embs.detach().cpu().numpy()
