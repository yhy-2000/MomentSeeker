...
sys.path.append('./languagebind/LanguageBind-main/')
...
def process_batch(video_path):
    return modality_transform['video'](video_path)
...
if __name__ == "__main__":
    ...
    model = LanguageBind(clip_type=clip_type).to(device).eval()
    model = torch.nn.DataParallel(model, device_ids=[0])
    tokenizer = LanguageBindImageTokenizer.from_pretrained(clip_type['image'])
    modality_transform = {c: transform_dict[c](model.module.modality_config[c]) for c in clip_type.keys()}


    output_dir = './languagebind/benchmark/test_grounding/result/languagebind/'
    os.makedirs(output_dir,exist_ok=True)
        ...
        try:
            ...
            if not ok:
                ...
                for i in tqdm(range(0, len(eval_dataset.index_examples), args.batch_size)):
                    ...
                    inputs = {'video': modality_transform['video'](batch_video)}

                    with torch.no_grad():
                        video_embed = model(inputs)['video']
                    index_embeddings.append(video_embed.float().detach().cpu().numpy())
                ...
                #     pool = mp.Pool()
                #     batch_video = eval_dataset.index_examples[i : i + args.batch_size]
                #     batch_video_tensor = tqdm(list(pool.map(process_batch, batch_video)))
                #     pool.close()
                #     pool.join()

                #     batch_video_tensor = torch.stack(batch_video_tensor, dim=0).cuda()
                #     print(batch_video_tensor.shape)

                #     inputs = {'video': batch_video_tensor}
                #     with torch.no_grad():
                #         video_embed = model(inputs)['video']
                #     index_embeddings.append(video_embed.float().detach().cpu().numpy())
                # index_embeddings = np.concatenate(index_embeddings, axis=0)
            ...
            for i in tqdm(range(0, len(eval_dataset.query_examples))):
                ...
                inputs = {}
                inputs['language'] = to_device(tokenizer([q.qtokens for q in batch], max_length=77, padding='max_length',truncation=True, return_tensors='pt'), device)

                with torch.no_grad():
                    text_emb = model(inputs)['language'].float().detach().cpu().numpy()

                if batch[0].qimage is not None:
                    if batch[0].qimage.endswith('.mp4'):
                        inputs = {'video': to_device(modality_transform['video']([q.qimage for q in batch]), device)}
                        with torch.no_grad():
                            video_emb = model(inputs)['video'].float().detach().cpu().numpy()
                            query_embeds = (video_emb + text_emb)/2
                    elif batch[0].qimage.endswith('.png') or batch[0].qimage.endswith('.jpg'):
                        inputs = {'image': to_device(modality_transform['image']([q.qimage for q in batch]), device)}
                        with torch.no_grad():
                            image_emb = model(inputs)['image'].float().detach().cpu().numpy()
                            query_embeds = (image_emb + text_emb)/2
                    else:
                        raise ValueError(f"qimage is of type {type(batch[0].qimage)}")
                else:
                    query_embeds = text_emb
