...
class MyVideoDataset(Dataset):
    def __init__(
        ...
        if isinstance(todo_ids, (str, Path)):
            todo_ids = my_get_ids(todo_ids)

        video_paths = todo_ids

        video_paths.sort()
        self.id2path = {pth.split('/')[-1]: pth for pth in video_paths}
        self.video_ids = list(self.id2path.keys())
        self.video_ids.sort()
...
def my_get_ids(path):
    suffix = Path(path).suffix
    # dict_keys(['qry_text', 'qry_img_path', 'qry_video_path', 'tgt_text', 'tgt_img_path', 'tgt_video_path'])
    if suffix == ".json" or suffix == ".jsonl":
        with open(path, "r") as f:
            data = json.load(f)
        ids = set()
        for item in data:
            ids.update(item['tgt_video_path'])
        ids = list(ids)
        ids.sort()
    else:
