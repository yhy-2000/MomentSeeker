# source activate /share/huaying/envs/rag/
import sys
...
if __name__ == "__main__":
    ...
    output_dir = './languagebind/benchmark/test_grounding/result/uniir/'
    os.makedirs(output_dir,exist_ok=True)
        ...
        try:
            ...
            if not ok:
                ...
                for i in tqdm(range(0, len(eval_dataset.index_examples), args.batch_size)):
                    ...
                    output_image = [Image.open(path) for path in batch_video]
                    ...
                    video_embed = model.encode_image(output_image_input.cuda())
            ...
            for i in tqdm(range(0, len(eval_dataset.query_examples))):
                ...
                multimodal_text = [q.qtokens for q in batch]
                if batch[0].qimage is not None:
                    multimodal_input_image = [Image.open(q.qimage) for q in batch]
                    ...
                    query_embeds = model.encode_multimodal_input(multimodal_text_input, multimodal_image_input, torch.ones(len(multimodal_text_input)).cuda(), torch.ones(len(multimodal_image_input)).cuda()).detach().cpu().numpy()
                    query_embeds = F.normalize(torch.from_numpy(query_embeds), dim=-1).numpy()
                else:
                    multimodal_text_input = tokenizer(multimodal_text).long().cuda()
                    query_embeds = model.encode_text(multimodal_text_input).detach().cpu().numpy()
                    query_embeds = F.normalize(torch.from_numpy(query_embeds), dim=-1).numpy()
