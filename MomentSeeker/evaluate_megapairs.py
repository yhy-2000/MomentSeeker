...
if __name__ == "__main__":
    ...
    model = AutoModel.from_pretrained("/share/huaying/pretrained_model/MMRet-base/", trust_remote_code=True) # You must set trust_remote_code=True
    model.set_processor("/share/huaying/pretrained_model/MMRet-base/")
    model.eval()
    model = model.cuda()

    output_dir = './languagebind/benchmark/test_grounding/result/mega_pairs/'
    os.makedirs(output_dir,exist_ok=True)
        ...
        # try:
            ...
            if not ok:
                ...
                for i in tqdm(range(0, len(eval_dataset.index_examples), args.batch_size)):
                    ...
                    video_embed = model.encode(images=batch_video)
            ...
            for i in tqdm(range(0, len(eval_dataset.query_examples))):
                ...
                truncated_text = [' '.join(q.qtokens.split()[:30]) for q in batch]
                query_embeds = model.encode(images=[q.qimage for q in batch], text=truncated_text).detach().cpu().numpy()
