...
class MagicLens(nn.Module):
  ...
  def clip_encode(self, input_batch: Dict) -> Tuple[jax.Array, jax.Array]:
    ...
    if input_batch['image']!=None:
      ...
      _, text_embs = self.clip(
          None, input_batch['ids'], normalize=False
      )
      return None, text_embs
  ...
  def __call__(self, input_batch: Dict) -> Dict:
    ...
    if img_embed is not None:
      ...
      txt_embed = txt_embed.reshape([-1, 1, txt_embed.shape[-1]])
      multimodal_embed = self.multimodal_encoder(  # [B, 1, D]
          txt_embed
          )
      multimodal_embed = self.contrastive_multimodal_pooler(
          multimodal_embed)
